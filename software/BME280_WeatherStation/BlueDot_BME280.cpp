...
float BlueDot_BME280::readTempC(void)
{
	...
	{
		int32_t adc_T;
		unsigned long alt = micros();
		adc_T = (uint32_t)readByte(BME280_TEMPERATURE_MSB) << 12;
		unsigned long neu = micros();
		Serial.print("Byte 1: "); Serial.println(neu - alt);
		alt = micros();
		adc_T |= (uint32_t)readByte(BME280_TEMPERATURE_LSB) << 4;
		neu = micros();
		Serial.print("Byte 2: "); Serial.println(neu - alt);
		alt = micros();
		adc_T |= (readByte(BME280_TEMPERATURE_XLSB) >> 4 )& 0b00001111;
		neu = micros();
		Serial.print("Byte 3: "); Serial.println(neu - alt);
		alt = micros();
		...
		T = T / 100;
		neu = micros();
		Serial.print("Rest: "); Serial.println(neu - alt);
		return T;
	}
}
