...
void loop() {
  if (rxOk) { // wenn die Interrupt-Routine rxOk auf true gesetzt hat, dann ist der Puffer mit den Daten gefuellt
     ...
     */
    if (xBits == 3 && !(randomID & 0x10)) { // wenn die xBits gesetzt sind und Bit 4 der Zufalls-ID Null ist (ist beim Wind- und Regensensor immer der Fall)
      if (DEBUG) {
      ...
      if (subID == 3) { // subID = 3 ist der Regensensor (absolute Regenmenge seit Batteriewechsel)
      ...
      if (DEBUG) Serial.print(F("zum PC senden: "));
      /*
